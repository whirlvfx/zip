...
//Crypto - Crypto struct
type Crypto struct {
	password []byte
	Keys     [3]uint32
}

//NewZipCrypto - NewZipCrypto function
func NewZipCrypto(passphrase []byte) *Crypto {
	z := &Crypto{}
...
func (z *Crypto) updateKeys(byteValue byte) {
...
//magicByte - magicByte function
func (z *Crypto) magicByte() byte {
	var t = z.Keys[2] | 2
	return byte((t * (t ^ 1)) >> 8)
}

//Encrypt - Encrypt function
func (z *Crypto) Encrypt(data []byte) []byte {
	length := len(data)
...
func (z *Crypto) Decrypt(chiper []byte) []byte {
	length := len(chiper)
...
//CryptoDecryptor - CryptoDecryptor function
func CryptoDecryptor(r *io.SectionReader, password []byte) (*io.SectionReader, error) {
...
//CryptoEncryptor - function CryptoEncryptor
func CryptoEncryptor(i io.Writer, pass passwordFn, fw *fileWriter) (io.Writer, error) {
	z := NewZipCrypto(pass())
